...
async function liveSnapshot() {
  ...
  const constituents = trackedCompanies.map((company, index) => {
    ...
    return { ...company, move: item.move, signal: item.move > 0.3 ? "Up today" : item.move < -0.3 ? "Down today" : "Near flat", filing: "Use NSE filings for result context" };
  });
  ...
  return {
    ...
    evidence: [{ id: "NIFTY-LIVE-QUOTE", claim: `NIFTY 50 quote at ${indexQuote.price.toLocaleString("en-IN")} (${indexQuote.move >= 0 ? "+" : ""}${indexQuote.move}% vs previous close)`, url: "https://finance.yahoo.com/quote/%5ENSEI", asOf: liveAsOf }],
